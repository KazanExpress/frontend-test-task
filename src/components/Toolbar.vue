...
export default {
  ...
  methods: {
    ...
    makeFileAndDownload() {
      const text = JSON.stringify(this.getTasks);
      const filename = `${this.Tabs.appName+this.Tabs.appID}.json`;
    ...
    fileEdit() {
      ...
      fileReader.onload = (e) => {
        const trg = e.target.result;
        // eslint-disable-next-line max-len
        if (/^[\],:{}\s]*$/.test(trg.replace(/\\["\\\/bfnrtu]/g, '@').replace(/"[^"\\\n\r]*"|true|false|null|-?\d+(?:\.\d*)?(?:[eE][+\-]?\d+)?/g, ']').
            replace(/(?:^|:|,)(?:\s*\[)+/g, ''))) {
          this.$store.commit('tasksListFromFile', JSON.parse(trg));
        } else {
